...
def login():
    if request.method == "POST":
        #getting the users input for login
        email = request.form.get('email')
        password = request.form.get('password')
        
        #checking if login email is valid
...
def reset_password():
    if request.method == "POST":
        #obtaining user input from website
        ...
        user = User.query.filter_by(email=email).first()
        if user:
            #updating password to new password
...
def change_user_data():
    if request.method == "POST":
        #obtaining user input from website and current database
        ...
        user = User.query.filter_by(email=email).first()
        if user:
            #no input means database not updated
            if len(new_email) <1:
                if len(new_name) <1:
